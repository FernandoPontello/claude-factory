...
A base é **Agile** (e não Scrum/Basic) pela hierarquia nativa **Epic > Feature > User Story**: a `task` canônica mapeia para **User Story** — a filha de nível-requisito da Feature, nativa e visível no board e no backlog. (Scrum usa *Product Backlog Item* no lugar de User Story; Basic não tem o nível portfólio.) O tempo **não** depende mais de campo nativo: User Story não tem `CompletedWork` (esse é de Task/Bug), então o tempo da factory degrada para comentário — ver §4.
...
- **Tempo é comentário (capability `time: comment`).** User Story não tem `CompletedWork` (campo de Task/Bug), então os minutos do `complete_task` viram um comentário com marcador canônico `⏱ factory: N min` — o mesmo padrão do Linear. O `## Tempo` do `task.md` segue sendo a verdade em minutos; o board é projeção.
